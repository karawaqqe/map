...
function WorldMap() {
	...
	useEffect(() => {
		const audioEntries = [
			...
			{ audio: windAudioRef.current, volume: 0.2 },
		].filter(({ audio }) => audio);
						...
						>
							...
							{hitboxes[continent.id] && (
								<path
									...
									focusable="true"
									onClick={
										CONTINENT_ROUTES[continent.id]
											? () => enterContinent(continent.id)
											: undefined
									}
									onKeyDown={(event) =>
										handleContinentKeyDown(event, continent.id)
									}
								/>
							)}
						</g>
						...
						{continents
							...
							.map((continent) => (
								...
								>
									...
									>
										<img
											className={styles.continentCrest}
											src={continent.crest}
											alt=""
										/>
